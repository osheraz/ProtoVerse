...
class MultiCamera(SensorBase):
    ...
    def __init__(self, cfg: CameraCfg):
        ...
        if self.cfg.spawn is not None:
            ...
            if self.cfg.spawn.vertical_aperture is None:
                self.cfg.spawn.vertical_aperture = (
                    self.cfg.spawn.horizontal_aperture
                    * self.cfg.height
                    / self.cfg.width
                )
        ...
        for i in range(self.cfg.cams_per_env):
            ...
            if len(matching_prims) == 0:
                raise RuntimeError(
                    f"Could not find prim with path {self.cfg.prim_path}{i}."
                )
    ...
    def set_intrinsic_matrices(
        ...
        for i, intrinsic_matrix in zip(env_ids, matrices):
            ...
            if (
                params["horizontal_aperture_offset"] > 1e-4
                or params["vertical_aperture_offset"] > 1e-4
            ):
                omni.log.warn(
                    "Camera aperture offsets are not supported by Omniverse. These parameters are ignored."
                )
    ...
    def set_world_poses_from_view(
        ...
        orientations = quat_from_matrix(
            create_rotation_matrix_from_view(
                eyes, targets, up_axis, device=self._device
            )
        )
    ...
    def _create_buffers(self):
        ...
        self._data.output = {}
        self._data.info = [
            {name: None for name in self.cfg.data_types}

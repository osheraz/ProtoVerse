...
from protoverse.envs.base_env.env_utils.terrains.subterrain_generator import (
...
# Full Terrain Map Layout (Top-Down View, Aligned with Code & Plot)
#
#         X → (Terrain Types → subterrain_idx)
#        ┌─────────────────────────────────────────────┐
#        │                  Top Border                 │
#        ├─────────────────────────────────────────────┤
#   ↑    │ +--------+--------+--------+--------+       │
#   Y    │ |        |        |        |        |       │
# (Levels│ | Flat   | Slope  | Stairs | Random | ← Level 0
#   ↓    │ |        |        |        |        |       │
#        │ +--------+--------+--------+--------+       │
#        │ |        |        |        |        |       │
#        │ | Flat   | Slope  | Stairs | Random | ← Level 1
#        │ |        |        |        |        |       │
#        │ +--------+--------+--------+--------+       │
#        │ |        |        |        |        |       │
#        │ | Flat   | Slope  | Stairs | Random | ← Level 2
#        │ |        |        |        |        |       │
#        ├─────────────────────────────────────────────┤
#        │              Bottom Border                  │
#        ├─────────────────────────────────────────────┤
#        │ Buffer Zone │    Object Playground          │
#        └─────────────────────────────────────────────┘
#
# Legend:
# - Each tile represents a sub-terrain generated by `curriculum()`
# - The layout is [num_levels × num_terrains] grid (Y × X)
# - Terrain types are selected by `choice = subterrain_idx / num_terrains`
#   and compared to cumulative `terrain_proportions`
# - Border, Buffer, and Object Playground are extra regions added to the map
#
# Coordinate Notes:
# - Y axis (rows) → level_idx (difficulty progression)
# - X axis (columns) → subterrain_idx (terrain type slot)
# - Indexing into `height_field_raw[y, x]` matches visual layout

...
class BaseEnv:
    ...
    def __init__(self, config, device: torch.device, *args, **kwargs):
        ...
        simulator_config: SimulatorConfig = SimulatorConfigClass.from_dict(
        ...
        if (
            self.config.motion_lib is not None
            and self.config.motion_lib.motion_file is not None
        ):
            self.motion_lib: MotionLib = self.instantiate_motion_lib()
            self.create_motion_manager()
        else:
            assert (
                self.state_init == self.StateInit.Default
            ), "Motion lib must be set and a motion file must be provided if state_init is not Default"
        ...
        self.force_respawn_on_flat = False  # TODO: make configurable
    ...
    def get_envs_respawn_position(
        ...
        if rigid_body_pos is not None:
            ...
            z_offset = (
                z_all_joints.gather(1, z_indices).view(-1, 1)
    ...
    def reset_default(self, env_ids):
        ...
        rigid_body_pos[:, :, :3] -= rigid_body_pos[:, 0, :3].unsqueeze(1).clone()
    ...
    def build_termination_heights(self):
        ...
        termination_heights = np.array(
            [termination_height] * self.simulator.robot_config.num_bodies
        )

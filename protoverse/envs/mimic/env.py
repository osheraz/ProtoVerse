...
from protoverse.envs.mimic.mimic_utils import (
    dof_to_local,
    exp_tracking_reward,
)
from protoverse.envs.base_env.env_utils.humanoid_utils import quat_diff_norm
...
from protoverse.envs.mimic.components.mimic_obs import MimicObs
from protoverse.envs.mimic.components.mimic_motion_manager import MimicMotionManager
from protoverse.envs.mimic.components.masked_mimic_obs import MaskedMimicObs
...
class Mimic(BaseEnv):
    ...
    def create_visualization_markers(self):
        ...
        if self.config.masked_mimic.enabled:
            ...
            future_body_markers_cfg = VisualizationMarker(
                type="sphere", color=(1.0, 1.0, 0.0), markers=future_body_markers
            )
    ...
    def get_markers_state(self):
        ...
        target_pos[..., -1:] += self.terrain.get_ground_heights(target_pos[:, 0]).view(
            self.num_envs, 1, 1
        )
    ...
    def compute_reward(self):
        ...
        ref_lr = dof_to_local(
            ref_state.dof_pos,
            self.simulator.robot_config.dof_offsets,
            self.simulator.robot_config.joint_axis,
            True,
        )
        ...
        relative_to_data_gt[:, :, -1:] -= self.terrain.get_ground_heights(
            gt[:, 0]
        ).view(self.num_envs, 1, 1)

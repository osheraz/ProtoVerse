...
class ViserCameraManager:
    ...
    def setup_gui(self, viewport_folder, controls_folder):
        ...
        with self.controls_folder:
            self.add_camera_button = self.viser_server.gui.add_button(
                label="Add Current View", icon=viser.Icon.CAMERA
            )
        ...
        if len(self.frustums) == self.max_cams_per_env:
            ...
            raise ValueError(
                "More frustums in camera_poses.h5 file than allowed by the MultiTiledCameraCfg"
            )
    ...
    def create_frustum(self, position, wxyz, camera_params=None):
        ...
        frustum = self.viser_server.scene.add_camera_frustum(
            ...
            fov=2
            * np.arctan(
                (camera_params["horizontal_aperture"] / 2)
    ...
    def save_camera_pose(self, position, wxyz):
        ...
        with h5py.File(self.camera_poses_file, "a") as f:
            if "poses" not in f:
                f.create_dataset(
                    "poses", data=pose[np.newaxis, :], maxshape=(None, 7), chunks=True
                )
            else:
                dset = f["poses"]
        ...
        try:
            ...
            with h5py.File(self.camera_poses_file, "a") as f:
                if "poses" in f:
                    ...
                    if len(new_poses) > 0:
                        f.create_dataset(
                            "poses", data=new_poses, maxshape=(None, 7), chunks=True
                        )
            ...
            try:
                ...
                if not hasattr(self, "tf_gizmo"):
                    ...
                    def _(_):
                        ...
                        self.input_vector_xyz[frustum.name].value = (
                            self.tf_gizmo.position.tolist()
                        )
                        self.input_vector_rpy[frustum.name].value = [
                            float(
                                SO3(self.tf_gizmo.wxyz).compute_roll_radians()
                                * 180
                                / np.pi
                            ),
                            float(
                                SO3(self.tf_gizmo.wxyz).compute_pitch_radians()
                                * 180
                                / np.pi
                            ),
                            float(
                                SO3(self.tf_gizmo.wxyz).compute_yaw_radians()
                                * 180
                                / np.pi
                            ),
                        ]
                ...
                with self.controls_folder:
                    ...
                    vector_input_xyz = self.viser_server.gui.add_vector3(
                        "XYZ", initial_value=frustum.position, step=0.1
                    )
                    ...
                    def _(_):
                        ...
                        with h5py.File(self.camera_poses_file, "a") as f:
                            if "poses" in f:
                                poses = f["poses"][:]
                                poses[camera_idx] = np.array(
                                    [*frustum.position, *frustum.wxyz]
                                )
                                del f["poses"]
                                f.create_dataset(
                                    "poses", data=poses, maxshape=(None, 7), chunks=True
                                )

...
from protoverse.simulator.isaaclab.utils.scene import SceneCfg
        ...
        if self._contact_sensor.data.force_matrix_w is not None:
            isaacsim_rb_contacts = (
                self._contact_sensor.data.force_matrix_w.clone().view(
                    self.num_envs, len(self._contact_sensor.body_names), -1, 3
                )
            )
    ...
    def render(self) -> None:
        ...
        if not self.headless:
            if not hasattr(self, "_perspective_view"):
                from protoverse.simulator.isaaclab.utils.perspective_viewer import (
                    PerspectiveViewer,
                )

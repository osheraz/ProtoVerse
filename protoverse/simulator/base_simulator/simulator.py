...
from protoverse.simulator.base_simulator.robot_state import RobotState, DataConversion
...
class Simulator(ABC):
    ...
    def build_body_ids_tensor(self, body_names: List[str]) -> torch.Tensor:
        ...
        for body_name in body_names:
            ...
            assert (
                body_id != -1
            ), f"Body part {body_name} not found in {self.robot_config.body_names}"
    ...
    def on_environment_ready(self) -> None:
        ...
        self._dof_limits_lower_common = self._dof_limits_lower_sim[
    ...
    def step(
        ...
        markers_state: Optional[Dict[str, MarkerState]] = None,
    ) -> None:
        """
    ...
    def reset_envs(
        self, new_states: Dict[str, torch.Tensor], env_ids: Optional[torch.Tensor]
    ) -> None:
        """
        ...
        self.init_states_on_resets[env_ids, ...] = new_states.root_pos
    ...
    def get_default_state(self) -> RobotState:
        ...
        simulator_default_state = simulator_default_state.convert_to_common(
    ...
    def get_bodies_contact_buf(
        ...
        """
        simulator_bodies_contact_forces = self._get_simulator_bodies_contact_buf(
            env_ids
        )
    ...
    def get_object_root_state(
        ...
        """
        simulator_object_root_state: RobotState = self._get_simulator_object_root_state(
            env_ids
        )
    ...
    def _create_legged_robot_tensors(self) -> None:
        ...
        p_gains: torch.Tensor = torch.zeros(
            self.robot_config.number_of_actions,
            dtype=torch.float,
            device=self.device,
            requires_grad=False,
        )
        d_gains: torch.Tensor = torch.zeros(
            self.robot_config.number_of_actions,
            dtype=torch.float,
            device=self.device,
            requires_grad=False,
        ...
        for i in range(self._num_dof):
            ...
            if not found:
                p_gains[i] = 0.0
                d_gains[i] = 0.0
                if self.robot_config.control.control_type in [
    ...
    def _process_dof_props(self) -> None:
        ...
        if self.robot_config.dof_effort_limits is not None:
            self._torque_limits_common = torch.tensor(
                self.robot_config.dof_effort_limits,
                dtype=torch.float,
                device=self.device,
                requires_grad=False,
            )
    ...
    def _compute_torques(self, action: torch.Tensor) -> torch.Tensor:
        ...
        if self.control_type == ControlType.PROPORTIONAL:
            ...
            torques = (
                ...
                * (dof_state.dof_vel - self.last_dof_vel)
                / self.dt
            )

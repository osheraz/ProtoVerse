            ...
            if value is None:
                ...
                # Handle Optional types
                    ...
                    if hasattr(inner_type, "from_dict"):
                        processed_dict[key] = (
                            inner_type.from_dict(value) if value is not None else None
                        )
                    else:
                ...
                print(
                    f"Note: '{key}' is not a field in {cls.__name__}, it will be ignored"
                )
...
class RobotAssetConfig(ConfigBuilder):
    ...
    asset_root: str = "protoverse/data/assets"
    ...
    def __post_init__(self):
        ...
        if not self.asset_file_name and not self.usd_asset_file_name:
            raise ValueError(
                "Either asset_file_name or usd_asset_file_name must be specified, depending on the simulator you are using"
            )
...
class InitState(ConfigBuilder):
    ...
    default_joint_angles: Optional[Dict[str, float]] = (
        None  # joint name -> angle in radians
    )
